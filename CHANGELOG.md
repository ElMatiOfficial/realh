...
- **KMS-backed signer** at [packages/server/src/services/signer/kms-gcp.js](packages/server/src/services/signer/kms-gcp.js). Delegates every signature to GCP Cloud KMS via `asymmetricSign`; the private key never enters the Node process. On boot: fetches the PEM public key, converts to JWK, caches, and runs a sign/verify self-test before accepting traffic. 8 unit tests with a mocked KMS client exercise the round-trip against real Ed25519 crypto. Enable with `KEY_MANAGER=kms KMS_PROVIDER=gcp KMS_KEY_ID=...`.
- **Signer abstraction** at [packages/server/src/services/signer/](packages/server/src/services/signer/) — common surface (`signCompact`, `signJwt`, `getPublicJwk`, `getKeyId`) used by `credentialService` and the JWKS route regardless of backend. Header `alg` / `kid` are enforced by the signer, not the caller.
...
- **Retired** `packages/server/src/services/keyManager.js` and the stub `kmsKeyManager.js`. Replaced by the signer abstraction in [services/signer/](packages/server/src/services/signer/). Breaking change to internal imports — no public API change; `/health`, `/.well-known/jwks.json`, credential shape, and HTTP responses are unchanged.
