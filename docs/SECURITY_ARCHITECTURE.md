...
| Guessing or replaying a live code | ~39-bit code space behind `liveCodeVerifyLimiter` (10 req/min/IP); single-use consume is atomic in every data layer; default 120 s TTL; only the SHA-256 of the code is stored; replay surfaces as an explicit `already_used` warning to the verifier. | The relay attack (scammer convinces the real owner to mint a code mid-call) is social, not cryptographic — mitigated by the context note shown at verification, not eliminated. |
